...
CREATE TABLE IF NOT EXISTS `yuju`.`members` (
  ...
  `edit_date` DATETIME NOT NULL COMMENT '수정 일자',
...
CREATE TABLE IF NOT EXISTS `yuju`.`inquiries` (
  `inquiry_id` INT NOT NULL AUTO_INCREMENT COMMENT '문의 ID',
  `member_id` INT NULL DEFAULT NULL COMMENT '회원 ID',
  ...
  `reg_date` DATETIME NOT NULL COMMENT '등록 일자',
  `edit_date` DATETIME NOT NULL COMMENT '변경 일자',
...
CREATE TABLE IF NOT EXISTS `yuju`.`payments` (
  ...
  `reg_date` DATETIME NOT NULL COMMENT '결제 일자',
  `edit_date` DATETIME NOT NULL COMMENT '수정 일자',
...
CREATE TABLE IF NOT EXISTS `yuju`.`room_imgs` (
  `room_img_id` INT NOT NULL AUTO_INCREMENT COMMENT '이미지 ID',
  `room_id` INT NOT NULL COMMENT '객실 ID',
  `img_url` VARCHAR(255) NOT NULL COMMENT '사진 URL',
  `reg_date` DATETIME NOT NULL COMMENT '등록 일자',
  `edit_date` DATETIME NOT NULL COMMENT '변경 일자',
