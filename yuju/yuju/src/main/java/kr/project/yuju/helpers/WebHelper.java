...
public class WebHelper {
     ...
     *            Exception으로 선언했으므로 어떤 하위 객체가 전달되더라고 형변환되어 받는다.
     */
    public void badRequerst(Exception e) {
        this.redirect(403, null, e.getMessage());
    }

    /**
     * 파라미터가 잘못된 경우 호출할 이전 페이지 이동 기능
     * 
     * @param message - 개발자가 직접 전달하는 에러 메시지
     */
    public void badRequerst(String message) {
        this.redirect(403, null, message);
    }

    /**
     * JAVA 혹은 SQL쪽에서 잘못된 경우 호출할 이전 페이지 이동 기능
     * 
     * @param e - 에러정보를 담고 있는 객체.
     *            Exception으로 선언했으므로 어떤 하위 객체가 전달되더라고 형변환되어 받는다.
     */
